        ...
        for m in msgList:
            ...
            try:
                with sql.connect("logintopic.db") as con:
                    cur = con.cursor()
                    cur.execute("INSERT INTO logintopic (user_ssn, user_name, behavior) VALUES (?,?,?)", (temp.get(
                        'user_ssn'), temp.get('user_name'), 'login'))
                    con.commit()
            except:
                con.rollback()
            finally:
        ...
        for m in msgList:
            ...
            try:
                with sql.connect("searchtopic.db") as con:
                    cur = con.cursor()
                    cur.execute("INSERT INTO searchtopic (user_ssn, user_name, behavior, book_name) VALUES(?,?,?,?)", (temp.get(
            ...
            try:
                with sql.connect("borrowtopic.db") as con:
                    ...
                    cur.execute("INSERT INTO borrowtopic (user_ssn, user_name, behavior, book_name) VALUES (?,?,?,?)", (temp.get(
                        'user_ssn'), temp.get('user_name'), 'borrow', temp.get('book_name')))
                    con.commit()
            except:
                con.rollback()
            finally:
...
def r_signin():
    ...
    cur.execute("SELECT * FROM readers WHERE rname=? and password=?",
                (rname, rpassword))
    people = cur.fetchall()
    if len(people) == 0:
        data = {
        ...
        return jsonify(data)
    cur.execute("SELECT ssn FROM readers WHERE rname=? and password=?",
                (rname, rpassword,))
    ssn = cur.fetchone()[0]
    session["reader"] = rname
    session["ssn"] = ssn
...
def r_signup():
    try:
        ...
        with sql.connect("readers.db") as con:
            cur = con.cursor()
            cur.execute("INSERT INTO readers (rname, ssn, address, mail, phone, password) VALUES (?,?,?,?,?,?)",
                        (rname, ssn, address, mail, phone, password))
            con.commit()
            msg = "讀者帳號已成功建立！"
    except:
        con.rollback()
...
def book_search():
    ...
    cur = con.cursor()
    cur.execute(
        "select * from books where title LIKE '%{}%' order by id desc".format(book_name))
    books = cur.fetchall()
    if book_name != '':
        ...
        if request.method == 'POST':
            try:
                ...
                with sql.connect("readers.db") as con:
                    ...
                    if address:
                        cur.execute(
                            "update readers set address=? WHERE rname=?", (address, session["reader"]))
                    if mail:
                        cur.execute("update readers set mail=? WHERE rname=?",
                                    (mail, session["reader"]))
                    if phone:
                        cur.execute("update readers set phone=? WHERE rname=?",
                                    (phone, session["reader"]))
                    if password:
                        cur.execute(
                            "update readers set password=? WHERE rname=?", (password, session["reader"]))
                    con.commit()
                    msg = "讀者資料修改成功！"
            except:
                con.rollback()
    ...
    if request.method == 'POST':
        try:
            ...
            with sql.connect("recommends.db") as con:
                cur = con.cursor()
                cur.execute("INSERT INTO recommends (ISBN, title, author, category, version) VALUES (?,?,?,?,?)",
                            (ISBN, title, author, category, version))
                con.commit()
                msg = "感謝您的推薦！我們會盡快購買！"
        except:
            con.rollback()
...
def s_signin():
    with sql.connect("staffs.db") as con:
        ...
        cur.execute(
            "SELECT * FROM staffs WHERE empid=? and password=?", (empid, spassword))
        people = cur.fetchall()
        if len(people) == 0:
            return redirect("/result?msg=帳號或密碼錯誤")
        cur.execute(
            "SELECT sname FROM staffs WHERE empid=? and password=?", (empid, spassword,))
...
def s_member():
    if "staff" in session:
        ...
        con.row_factory = sql.Row
        cur = con.cursor()
        cur.execute("SELECT sname FROM staffs WHERE empid=?",
                    (session["staff"],))
        sname = cur.fetchone()[0]
        con.close()
        return render_template("/s_member.html", sname=sname)
...
def all():
    ...
    def super_frquent_user():
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT user_ssn, user_name, COUNT(user_ssn) AS num FROM logintopic GROUP BY user_ssn ORDER BY COUNT(user_ssn) DESC")
        data = cur.fetchall()
        con.close()
        for row in data:
            print(row["user_ssn"])
    ...
    def super_book_borrowed():
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT book_name, COUNT(book_name) as num FROM borrowtopic GROUP BY book_name ORDER BY COUNT(book_name) DESC")
        data = cur.fetchall()
        con.close()
        for row in data:
            print(row["book_name"])
    ...
    def super_book_searched():
        ...
        cur = con.cursor()
        cur.execute(
            "SELECT book_name, COUNT(book_name) as num FROM searchtopic GROUP BY book_name ORDER BY COUNT(book_name) DESC")
        data = cur.fetchall()
        con.close()
        for row in data:
            print(row["book_name"])
